    ...
    public sealed class TitleScreen : MonoBehaviour
    {
        ...
        void ShowHowTo()
        {
            ...
            string text =
                ...
                "   손패에 특정 카드 조합이 모이면 저절로 합쳐져 뽑기로는 못 얻는 히든 유닛이 된다 (합성표 → 손패 조합).\n" +
                ...
                "■ 조작  B = 건설 목록(글자 키로 타워 선택) 또는 상점 클릭 → 커서에 뜬 타워를 빈 칸에 클릭 (시프트 = 연속, 우클릭/ESC = 취소)\n" +
                "   D = 뽑기, F = 레벨업 경험치, 카드 위에서 W = 보내기, 타워 클릭 = 강화/판매/합치기/합성, 우클릭 = 판매,\n" +
                "   스페이스 = 정지, 1/2/3 = 배속, ESC = 취소/메뉴.";
