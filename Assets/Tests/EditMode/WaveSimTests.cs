...
namespace LaneBattle.Tests
{
    ...
    {
        static LaneSim Manual(ulong seed = 1, int width = 4) => new LaneSim(new LaneConfig { AutoWaves = false, Width = width }, seed);
        ...
        public void SameSeedSameResult()
        {
            ...
            {
                var s = Manual(42);
                s.Build(WaveCatalog.Tower(1), 0, 0); s.Build(WaveCatalog.Tower(5), 1, 1); s.Build(WaveCatalog.Tower(7), 2, 2);
        ...
        {
            var s = Manual(3);
            for (int c = 0; c < 4; c++) s.Build(WaveCatalog.Tower(2), c, 0); // 가시 덤불 앞줄 전부
            Instant(s);
        ...
        {
            var s = Manual(7);
            for (int c = 0; c < 4; c++) s.Build(WaveCatalog.Tower(7), c, 1); // 포탑, 대공 불가
            Instant(s);
            ...
            var t = Manual(7);
            for (int c = 0; c < 4; c++) t.Build(WaveCatalog.Tower(1), c, 1); // 나무 궁수, 대공
            Instant(t);
        ...
        {
            var s = Manual(5);
            var tower = s.Build(WaveCatalog.Tower(7), 1, 0);
        ...
        {
            var cfg = new LaneConfig { AutoWaves = false, GridStartX = 1 }; // 타워를 입구 바로 옆에
            var s = new LaneSim(cfg, 9);
            for (int c = 0; c < 4; c++) s.Build(WaveCatalog.Tower(7), c, 0);
            Instant(s);
        ...
        {
            var s = Manual(11);
            var t = s.Build(WaveCatalog.Tower(7), 1, 0);
            Instant(s);
            t.SilenceLeft = 100;
            s.Send(WaveCatalog.Attacker(4));
            int attacks = 0;
        ...
        {
            var s = Manual(13);
            for (int c = 0; c < 4; c++) s.Build(WaveCatalog.Tower(5), c, 1);
            Instant(s);
        ...
        public void AllBaseWavesTerminate()
        {
            for (int w = 0; w < WaveCatalog.BaseWaves.Length; w++)
            {
                ...
                s.Build(WaveCatalog.Tower(1), 0, 0); s.Build(WaveCatalog.Tower(6), 1, 0); s.Build(WaveCatalog.Tower(8), 2, 0); s.Build(WaveCatalog.Tower(1), 3, 0);
                s.Build(WaveCatalog.Tower(5), 1, 1); s.Build(WaveCatalog.Tower(7), 2, 1); s.Build(WaveCatalog.Tower(3), 1, 2);
                Instant(s);
                s.SpawnBaseWave(w);
                s.RunUntilQuiet();
